...
import { useRoqPlatformUserProfiles, useNewTableCount, useCarWashCount } from 'lib/roq';

import { CarWashListPage as ShowCaseList } from 'pages/car-washes';

function DashboardPage() {
  const { session } = useSession();

  const newTableStat = useNewTableCount();

  const carWashStat = useCarWashCount();

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'New Tables',
      result: newTableStat?.data,
      isLoading: newTableStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Car Washes',
      result: carWashStat?.data,
      isLoading: carWashStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`car-washes`} passHref legacyBehavior>
                  <Button
